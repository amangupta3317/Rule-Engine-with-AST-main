...
function evaluate(node, data) {
  if (node.type === 'operator') {
      ...
      return node.operator === 'AND' ? left && right : left || right; // Simplified logic
  } else if (node.type === 'operand') {
      let { key, operator, value } = node;

      // Trim and parse the value for accurate comparisons
      ...
      // Try to cast values to numbers when possible
      const parsedValue = isNaN(value) ? value : Number(value);
      
      // Log comparison details
      console.log(`Comparing: ${dataValue} ${operator} ${parsedValue}`);
      ...
      switch (operator) {
          ...
          case '<=':
              return dataValue <= parsedValue;
          case '==':
              return dataValue == parsedValue; // Use '==' for loose comparison
          case '!=':
              return dataValue != parsedValue;
          case '=': // Handle '=' as '=='
              return dataValue == parsedValue;
